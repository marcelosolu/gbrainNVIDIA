...
version: 0.46.3
feature_pitch:
  headline: "ZeroEntropy shuts down 2026-09-04 — switch embedding + reranker before then. New default: NVIDIA."
  one_liner: "One command migrates the brain (embeddings AND reranker): gbrain migrate embeddings --to nvidia:nv-embed-v1 --dim 1024"
...
- **Set and equal to the target** (e.g. `nvidia:nv-embed-v1` / `1024`): the
...
**Preferred — NVIDIA** (`NVIDIA_API_KEY` in env, or `nvidia_api_key` in
`~/.gbrain/config.json`). Free key at https://build.nvidia.com. To set the
key: `export NVIDIA_API_KEY=nvapi-…` or edit
`~/.gbrain/config.json` directly — do NOT use `gbrain config set
nvidia_api_key` (that writes the DB plane, which the embedding pipeline never
reads).

```bash
gbrain migrate embeddings --to nvidia:nv-embed-v1 --dim 1024 --dry-run   # cost preview — show the user
gbrain migrate embeddings --to nvidia:nv-embed-v1 --dim 1024 --yes
```

Note: **1280 is not a valid NVIDIA width** (valid: 1024/2048/4096), so this
...
**Neither key** — get one of the three (NVIDIA: https://build.nvidia.com — free),
or self-host (below).
